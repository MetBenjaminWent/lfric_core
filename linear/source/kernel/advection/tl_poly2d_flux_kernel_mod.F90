...
!-----------------------------------------------------------------------------
!
!> @brief Tangent linear for horizontal fluxes with 2D reconstruction.
!> @details The nonlinear model is:
!>          \f F = \rho u \f
!>          The tangent linear model is:
!>          \f F = \rho ls_u + ls_\rho u \f
module tl_poly2d_flux_kernel_mod
...
type, public, extends(kernel_type) :: tl_poly2d_flux_kernel_type
  private
...
contains
  procedure, nopass :: tl_poly2d_flux_code
end type

!-------------------------------------------------------------------------------
! Contained functions/subroutines
!-------------------------------------------------------------------------------
public tl_poly2d_flux_code

contains

!> @brief Computes the tangent linear for horizontal fluxes.
!! @param[in] nlayers    Number of layers
!! @param[inout] flux    ACTIVE Change in mass flux field
!! @param[in] wind       ACTIVE Change in wind field
!! @param[in] density    ACTIVE Tracer density
!! @param[in] ls_wind    Linearisation state for wind
!! @param[in] ls_density Linearisation state for tracer density
...
!! @param[in] undf_w3    Number of unique degrees of freedom for density field
...
!!                       reference element horizontal "outward faces"
subroutine tl_poly2d_flux_code( nlayers,              &
                                flux,                 &
                                wind,                 &
                                density,              &
                                ls_wind,              &
  ...
  do df = 1,nfaces_re_h
    v_dot_n(df) =  dot_product( basis_w2(:,df,df), &
                                outward_normals_to_horizontal_faces(:,df) )
  end do
...
end subroutine tl_poly2d_flux_code

end module tl_poly2d_flux_kernel_mod
